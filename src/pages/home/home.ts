 ...
 @ViewChild('searchbar', {read: ElementRef}) searchbar: ElementRef;
  ...
  loadMaps(){
      if(!!google) {
        ...
        this.errorAlert('Fehler', 'Fehler beim laden der Daten. Bitte prüfe deine Internet Verbindung.')
      }
  }
  ...
  getCurrentPositionfromStorage(markers){
      this.storage.get('lastLocation').then((result) => {
        if(result){
            ...
            let marker = this.addMarker(myPos, "Mein zuletzt gespeicherter Standort: " + result.location);
  ...
  choosePosition(){
    this.storage.get('lastLocation').then((result) => {
      if(result){
        ...
        title: 'Letzer Standort: ' + result.location,
        buttons: [
          {
            text: 'Aktualisieren',
            handler: () => {
          ...
          },
          {
            ...
            handler: () => {
              ...
              this.toastCtrl.create('Standort wurde gelöscht!');
              this.initializeMap();
              actionSheet.dismiss();
            }
          },
  ...
  getCurrentPosition() {
    this.loading = this.loadingCtrl.create({
      content: 'Dein Standort wird gesucht ...'
    });
    ...
    Geolocation.getCurrentPosition(locationOptions).then(
      (position) => {
          this.loading.dismiss().then(() => {
            ...
            let toast = this.toastCtrl.create({
              message: 'Position erfolgreich gefunden!',
              duration: 3000
            });
            ...
            setTimeout(() => {
              let alert = this.alertCtrl.create({
                title: 'Standort',
                message: 'Möchtest du deinen aktuellen Standort speichern?',
                buttons: [
                  {
                    text: 'Abbrechen'
                  },
                  {
                    ...
                    handler: data => {
                      ...
                      this.storage.set('lastLocation', lastLocation).then(()=>{
                        this.toastCtrl.create('Standort hinterlegt');
                      })
                    }
                  }
                ]
      ...
      (error) =>{
        this.loading.dismiss().then(() => {
          // Toast Info
          let toast = this.toastCtrl.create({
            message: 'Position nicht gefunden! Bitte aktivieren Sie ihr GPS.',
            duration: 3000
          });
          toast.present();
